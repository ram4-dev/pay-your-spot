...
import { listContactRecords,markContacted } from "@/lib/auction/service";
...
export function GET(request:Request){
  ...
  return Response.json({generatedAt:new Date().toISOString(),contacts:listContactRecords(getAuctionDatabase())},{headers:{"Cache-Control":"no-store"}});
}
...
export async function POST(request:Request){
  ...
  const changes=markContacted(getAuctionDatabase(),body.bidId);
