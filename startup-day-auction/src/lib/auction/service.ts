...
type BidRow={id:string;spot_id:string;bidder_company:string;bidder_email:string;bidder_logo:Uint8Array|null;logo_mime_type:"image/png"|"image/jpeg"|null;amount_cents:number;status:BidStatus;created_at:string;contacted_at:string|null};
...
function getSpotRanking(database:AuctionDatabase,spot:SpotRow):PublicBid[]{if(!spot.started_at)return[];const rows=database.prepare("SELECT id,bidder_company,logo_mime_type,amount_cents,status,created_at FROM bids WHERE spot_id=? AND created_at>=? AND status IN('LEADING','OUTBID','RESERVED','CONTACTED') ORDER BY amount_cents DESC,created_at ASC LIMIT 50").all(spot.id,spot.started_at) as Array<{id:string;bidder_company:string;logo_mime_type:string|null;amount_cents:number;status:BidStatus;created_at:string}>;return rows.map((bid,index)=>({id:bid.id,rank:index+1,company:bid.bidder_company,logoUrl:bid.logo_mime_type?`/api/logos/${bid.id}`:null,amountCents:bid.amount_cents,status:bid.status,createdAt:bid.created_at}));}

export function getAuctionState(database:AuctionDatabase,now=new Date()):AuctionState{
  closeExpiredAuctions(database,now);
  const rows=database.prepare("SELECT s.*,b.bidder_company,b.logo_mime_type,b.amount_cents FROM spots s LEFT JOIN bids b ON b.id=s.leading_bid_id ORDER BY s.rowid").all() as Array<SpotRow&{bidder_company:string|null;logo_mime_type:string|null;amount_cents:number|null}>;
...
export function getBidLogo(database:AuctionDatabase,bidId:string){return database.prepare("SELECT bidder_logo,logo_mime_type FROM bids WHERE id=?").get(bidId) as {bidder_logo:Uint8Array|null;logo_mime_type:"image/png"|"image/jpeg"|null}|undefined;}
function mapInternalBid(row:BidRow):InternalBid{return{id:row.id,spotId:row.spot_id,company:row.bidder_company,email:row.bidder_email,logoUrl:row.logo_mime_type?`/api/logos/${row.id}`:null,amountCents:row.amount_cents,status:row.status,createdAt:row.created_at,contactedAt:row.contacted_at};}
